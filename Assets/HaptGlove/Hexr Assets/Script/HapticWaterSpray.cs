...
{
    public class HapticWaterSpray : MonoBehaviour, IHandGrabUseDelegate
    {
        ...
        private bool PressureIncreased = true;
        #region input
        ...
        {
            if(strength > 0.6 && PressureIncreased == false)
            {
                PressureIncreased = true;
                RightpressureTrackerMain.TriggerSingleHapticsIncrease(HaptGlove.Haptics.Finger.Index,true,0.7f,1,true);
                LeftpressureTrackerMain.TriggerSingleHapticsIncrease(HaptGlove.Haptics.Finger.Index, true, 0.7f, 1, true);
            }
            else if(strength <= 0.4 && PressureIncreased == true)
            {
                PressureIncreased = false;
                RightpressureTrackerMain.RemoveIndexHaptics();
                LeftpressureTrackerMain.RemoveIndexHaptics();
            }
